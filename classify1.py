...
def main(args):
    ...
    for _, files in filelist.items():
        for ifile in files:
            ...
            t4.Branch('LeadJet_btagDeepFLV', leadJetBtagdeepFlv_2,'LeadJet_btagDeepFLV/F')#
            t4.Branch('DR_LepleadJet', drlepleadjet_2,'DR_LepleadJet/F')#
            ...
            for _ in range(len(out)): #looping only over the indices in input df
               ...
               # print (out.index[evt_index], getVariable(out, out.index[evt_index], 'Event_Flag'), getVariable(out, out.index[evt_index], 'lep_pt'))  
                if getVariable(out, out.index[evt_index], 'Event_Flag') == 13: #for Muon events 
                    st_1[0] = getVariable(out, out.index[evt_index], 'st')
                    ht_1[0] = getVariable(out, out.index[evt_index], 'ht')
                    nBtagMeddeepFlv_1[0] = getVariable(out, out.index[evt_index], 'nBtagMed_deepFLV')
                    NN_sig_mu[0] = getGuess(out, out.index[evt_index])
                    if NN_sig_mu[0] < 0.9:
                        lepPt_1[0] = getVariable(out, out.index[evt_index], 'lep_pt')
                        leadjetPt_1[0] = getVariable(out, out.index[evt_index], 'leadjet_pt')
                        met_1[0] = getVariable(out, out.index[evt_index], 'met')
                        lepEta_1[0] = getVariable(out, out.index[evt_index], 'lep_eta')
                        Ncentraljets_1[0] = getVariable(out, out.index[evt_index], 'Ncentraljets')
                        dphilepMet_1[0] = getVariable(out, out.index[evt_index], 'dphi_lepMet')
                        dphilepleadjet_1[0] = getVariable(out, out.index[evt_index], 'dphi_lepleadJet')
                        drlepclosestjet_1[0] = getVariable(out, out.index[evt_index], 'dr_lepclosestJet')
                        bvswratio_1[0] = getVariable(out, out.index[evt_index], 'bV_sW_ratio')
                        anglemujetmet_1[0] = getVariable(out, out.index[evt_index], 'angle_mujet_met')
                        t1.Fill()
                    fout.cd()
                    t3.Fill()
                    fout.cd()

                elif getVariable(out, out.index[evt_index], 'Event_Flag') == 11: #for Electron events
                    st_2[0] = getVariable(out, out.index[evt_index], 'st')
                    ht_2[0] = getVariable(out, out.index[evt_index], 'ht')
                    nBtagMeddeepFlv_2[0] = getVariable(out, out.index[evt_index], 'nBtagMed_deepFLV')
                    NN_sig_ele[0] = getGuess(out, out.index[evt_index])
                    if NN_sig_ele[0] < 0.9:
                        lepPt_2[0] = getVariable(out, out.index[evt_index], 'lep_pt')
                        leadjetPt_2[0] = getVariable(out, out.index[evt_index], 'leadjet_pt')
                        met_2[0] = getVariable(out, out.index[evt_index], 'met')
                        lepEta_2[0] = getVariable(out, out.index[evt_index], 'lep_eta')
                        Ncentraljets_2[0] = getVariable(out, out.index[evt_index], 'Ncentraljets')
                        dphilepMet_2[0] = getVariable(out, out.index[evt_index], 'dphi_lepMet')
                        dphilepleadjet_2[0] = getVariable(out, out.index[evt_index], 'dphi_lepleadJet')
                        drlepclosestjet_2[0] = getVariable(out, out.index[evt_index], 'dr_lepclosestJet')
                        bvswratio_2[0] = getVariable(out, out.index[evt_index], 'bV_sW_ratio')
                        anglemujetmet_2[0] = getVariable(out, out.index[evt_index], 'angle_mujet_met')
                        t2.Fill()
                    fout.cd()
                    t4.Fill()
                    fout.cd()
